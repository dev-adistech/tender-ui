...
import { ModelMastService } from 'src/app/Service/Master/model-mast.service';
import PerfectScrollbar from 'perfect-scrollbar';
import Swal from 'sweetalert2';

@Component({
  selector: 'app-model-mast',
  templateUrl: './model-mast.component.html',
  styleUrls: ['./model-mast.component.css']
})
export class ModelMastComponent implements OnInit {
  ...
  constructor(
    private toastr: ToastrService,
		...
		private ModelMastServ: ModelMastService,
  ) { 
    let op = this
		this.columnDefs = [
			{
  ...
  async ngOnInit() {
		...
		this.PER = await this._FrmOpePer.UserFrmOpePer('ModelMastComponent')
  ...
  Save(){
    if (this.PASS != this.PASSWORD) {
		...
		let SaveObj = {
			...
			M_NAME: this.M_NAME.trim(),
			ORD: this.ORD ? this.ORD : 0,
		}

		this.spinner.show()
		this.ModelMastServ.ModelMastSave(SaveObj).subscribe((SaveRes) => {
			try {
  ...
  LoadGridData(){
    this.spinner.show()
		this.ModelMastServ.ModelMastFill({}).subscribe((FillRes) => {
			try {
  ...
  onGridRowClicked(eve: any) {
    if (eve.event.target !== undefined) {
			...
			if (actionType == "DeleteData") {
				Swal.fire({
					title:
						"Are you sure you want to delete Model code " +
						eve.data.M_CODE +
						"?",
					icon: "warning",
					cancelButtonText: "No",
					...
					if (result.value) {
						this.spinner.show()
						this.ModelMastServ.ModelMastDelete({
							...
							try {
								if (ColDelRes.success == true) {
									...
									this.spinner.hide()
									this.toastr.warning(
										"Something went to wrong while delete Model code"
									)
								}
							} catch (err) {
								this.spinner.hide()
				...
				this.M_CODE = eve.data.M_CODE ? eve.data.M_CODE : 0
				this.M_NAME = eve.data.M_NAME ? eve.data.M_NAME : ""
