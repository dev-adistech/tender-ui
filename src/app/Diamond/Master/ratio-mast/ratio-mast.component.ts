...
import { RatoMastService } from 'src/app/Service/Master/rato-mast.service';
import Swal from 'sweetalert2';
import PerfectScrollbar from 'perfect-scrollbar';

@Component({
  selector: 'app-ratio-mast',
  templateUrl: './ratio-mast.component.html',
  styleUrls: ['./ratio-mast.component.css']
})
export class RatioMastComponent implements OnInit {
  ...
  constructor(
    private toastr: ToastrService,
		...
		private RatioMastSer: RatoMastService,
  ...
  async ngOnInit() {
		...
		this.PER = await this._FrmOpePer.UserFrmOpePer('RatioMastComponent')
  ...
  Save() {
		...
		let SaveObj = {
			RAT_CODE: this.RAT_CODE ? this.RAT_CODE : 0,
			ORD: this.ORD ? this.ORD : 0,
		}

		this.spinner.show()
		this.RatioMastSer.RatioMastSave(SaveObj).subscribe((SaveRes) => {
			try {
  ...
  LoadGridData() {
		this.spinner.show()
		this.RatioMastSer.RAtioMastFill({}).subscribe((FillRes) => {
			try {
  ...
  onGridRowClicked(eve: any) {
		if (eve.event.target !== undefined) {
			...
			if (actionType == "DeleteData") {
				Swal.fire({
					title:
						"Are you sure you want to delete Ratio code " +
						eve.data.RAT_CODE +
						"?",
					icon: "warning",
					cancelButtonText: "No",
					...
					if (result.value) {
						this.spinner.show()
						this.RatioMastSer.RatioMastDelete({
							RAT_CODE: eve.data.RAT_CODE,
						}).subscribe((ColDelRes) => {
							try {
								if (ColDelRes.success == true) {
									...
									this.spinner.hide()
									this.toastr.warning(
										"Something went to wrong while delete Ratio code"
									)
								}
							} catch (err) {
								this.spinner.hide()
				...
				this.RAT_CODE = eve.data.RAT_CODE ? eve.data.RAT_CODE : 0
