...
import { MacComMastService } from 'src/app/Service/Master/mac-com-mast.service';
import Swal from 'sweetalert2';
import PerfectScrollbar from 'perfect-scrollbar';

@Component({
  selector: 'app-mac-com-mast',
  templateUrl: './mac-com-mast.component.html',
  styleUrls: ['./mac-com-mast.component.css']
})
export class MacComMastComponent implements OnInit {
  ...
  constructor(
    private toastr: ToastrService,
		...
		private MacComMastServ: MacComMastService,
  ...
  async ngOnInit() {
		...
		this.PER = await this._FrmOpePer.UserFrmOpePer('MacComMastComponent')
  ...
  Save() {
		...
		if (!this.ALLOWINS) {
		  ...
		  if(!this.MCOM_NAME){
			this.toastr.warning('Enter Valid Comment')
			return
		  }
		...
		let SaveObj = {
			MCOM_NAME: this.MCOM_NAME.trim(),
			ORD: this.ORD ? this.ORD : 0,
		}

		this.spinner.show()
		this.MacComMastServ.MachComMastSave(SaveObj).subscribe((SaveRes) => {
			try {
  ...
  LoadGridData() {
		this.spinner.show()
		this.MacComMastServ.MachComMastFill({}).subscribe((FillRes) => {
			try {
		...
		if (eve.event.target !== undefined) {
			...
			if (actionType == "DeleteData") {
				Swal.fire({
					...
					if (result.value) {
						this.spinner.show()
						this.MacComMastServ.MachComMastDelete({
							MCOM_NAME: eve.data.MCOM_NAME,
						}).subscribe((ColDelRes) => {
							try {
				...
				this.MCOM_NAME = eve.data.MCOM_NAME ? eve.data.MCOM_NAME : ""
