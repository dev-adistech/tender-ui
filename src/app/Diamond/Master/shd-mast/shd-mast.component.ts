...
import { ShdMastService } from 'src/app/Service/Master/shd-mast.service';
...
export class ShdMastComponent implements OnInit {
  ...
  async ngOnInit() {
		...
		this.PER = await this._FrmOpePer.UserFrmOpePer('ShdMastComponent')
  ...
  Save() {
		...
		let SaveObj = {
			SH_CODE: this.SH_CODE ? this.SH_CODE : 0,
			SH_NAME: this.SH_NAME.trim(),
  ...
  onGridRowClicked(eve: any) {
		if (eve.event.target !== undefined) {
			...
			if (actionType == "DeleteData") {
				Swal.fire({
					title:
						"Are you sure you want to delete Shade code " +
						eve.data.SH_CODE +
						"?",
					icon: "warning",
					cancelButtonText: "No",
					...
					if (result.value) {
						this.spinner.show()
						this.ShdMastServ.ShdMastDelete({
							...
							try {
								if (ColDelRes.success == true) {
									...
									this.spinner.hide()
									this.toastr.warning(
										"Something went to wrong while delete Shade code"
									)
								}
							} catch (err) {
								this.spinner.hide()
				...
				this.SH_CODE = eve.data.SH_CODE ? eve.data.SH_CODE : 0
