...
export class ShpMastComponent implements OnInit {
  ...
  async ngOnInit() {
    ...
    this.PER = await this._FrmOpePer.UserFrmOpePer('ShpMastComponent')
  ...
  Save() {
    ...
    if (this._validator.isBlank(this.S_CODE, 'Shape Code')) return
    if (this._validator.isBlank(this.S_NAME, 'Shape Name')) return
    if (this._validator.isBlank(this.ORD, 'Order')) return
