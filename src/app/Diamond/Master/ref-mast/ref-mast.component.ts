...
import { ReflectionMastService } from 'src/app/Service/Master/reflection-mast.service';

@Component({
  selector: 'app-ref-mast',
  templateUrl: './ref-mast.component.html',
  styleUrls: ['./ref-mast.component.css']
})
export class RefMastComponent implements OnInit {
  ...
  constructor(
    private toastr: ToastrService,
		...
		private RefMastServ: ReflectionMastService,
  ...
  async ngOnInit() {
		...
		this.PER = await this._FrmOpePer.UserFrmOpePer('RefMastComponent')
  ...
  Save() {
		...
		let SaveObj = {
			REF_CODE: this.REF_CODE ? this.REF_CODE : 0,
			REF_NAME: this.REF_NAME.trim(),
			ORD: this.ORD ? this.ORD : 0,
		}

		this.spinner.show()
		this.RefMastServ.ReflectionMastSave(SaveObj).subscribe((SaveRes) => {
			try {
  ...
  LoadGridData() {
		this.spinner.show()
		this.RefMastServ.ReflectionMastFill({}).subscribe((FillRes) => {
			try {
  ...
  onGridRowClicked(eve: any) {
		if (eve.event.target !== undefined) {
			...
			if (actionType == "DeleteData") {
				Swal.fire({
					title:
						"Are you sure you want to delete Reflection code " +
						eve.data.REF_CODE +
						"?",
					icon: "warning",
					cancelButtonText: "No",
					...
					if (result.value) {
						this.spinner.show()
						this.RefMastServ.ReflectionMastDelete({
							...
							try {
								if (ColDelRes.success == true) {
									...
									this.spinner.hide()
									this.toastr.warning(
										"Something went to wrong while delete Reflection code"
									)
								}
							} catch (err) {
								this.spinner.hide()
				...
				this.REF_CODE = eve.data.REF_CODE ? eve.data.REF_CODE : 0
				this.REF_NAME = eve.data.REF_NAME ? eve.data.REF_NAME : ""
