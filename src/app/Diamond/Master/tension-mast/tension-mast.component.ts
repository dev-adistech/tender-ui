...
@Component({
  selector: 'app-tension-mast',
  templateUrl: './tension-mast.component.html',
  styleUrls: ['./tension-mast.component.css']
})
export class TensionMastComponent implements OnInit {
  ...
  async ngOnInit() {
    ...
    this.spinner.hide()
		...
		this.PER = await this._FrmOpePer.UserFrmOpePer('TensionMastComponent')
  ...
  Save() {
    if (this.PASS != this.PASSWORD) {
		...
		let SaveObj = {
			TEN_CODE: this.TEN_CODE,
			TEN_NAME: this.TEN_NAME.trim(),
  ...
  onGridRowClicked(eve: any) {
    if (eve.event.target !== undefined) {
			...
			if (actionType == "DeleteData") {
				Swal.fire({
					title:
						"Are you sure you want to delete Tension code " +
						eve.data.T_CODE +
						"?",
					icon: "warning",
					cancelButtonText: "No",
					...
					if (result.value) {
						this.spinner.show()
						this.TenMastServ.TenMastDelete({
							TEN_CODE: eve.data.T_CODE,
						}).subscribe((ColDelRes) => {
							try {
								if (ColDelRes.success == true) {
									...
									this.spinner.hide()
									this.toastr.warning(
										"Something went to wrong while delete Tension code"
									)
								}
							} catch (err) {
								this.spinner.hide()
				...
				this.TEN_CODE = eve.data.T_CODE ? eve.data.T_CODE : ""
				this.TEN_NAME = eve.data.T_NAME ? eve.data.T_NAME : ""
