...
export class ColMastComponent implements OnInit {
	...
	async ngOnInit() {
		...
		this.PER = await this._FrmOpePer.UserFrmOpePer('ColMastComponent')
	...
	onGridRowClicked(eve: any) {
		if (eve.event.target !== undefined) {
			...
			if (actionType == "DeleteData") {
				Swal.fire({
					title:
						"Are you sure you want to delete Color code " +
						eve.data.C_CODE +
						"?",
					icon: "warning",
					cancelButtonText: "No",
					...
					if (result.value) {
						this.spinner.show()
						this.ColMastServ.ColMastDelete({
				...
				this.C_CODE = eve.data.C_CODE ? eve.data.C_CODE : 0
				this.C_NAME = eve.data.C_NAME ? eve.data.C_NAME : ""
				this.C_ORD = eve.data.ORD ? eve.data.ORD : 0
			}
		}
	}
