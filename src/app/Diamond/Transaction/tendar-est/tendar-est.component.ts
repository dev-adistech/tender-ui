...
export class TendarEstComponent implements OnInit {
  ...
  CHANGE(eve){
    console.log("212",this.ROUNDC1)
    console.log("213",this.selectedSz)
    console.log("214",this.FANCY1)
  }
    ...
    let _GridRowData1= []
      ...
      for(let i =0;i<_GridRowData1.length;i++){
        if(_GridRowData1[i].PLANNO === params.data.PLANNO && _GridRowData1[i].SRNO == params.data.SRNO && _GridRowData1[i].PTAG === params.data.PTAG){
          _GridRowData1[i].C_CODE = params.data.CARAT
        }
      }
 ...
 w3_close() {
  ...
  onCellDoubleClicked(eve){
    ...
    //   if(eve.data.DETID === this.DETID && this.COMP_CODE === this.MAinGridData[i].COMP_CODE && eve.data.SRNO === this.MAinGridData[i].SRNO){
    //     Newarray.push(this.MAinGridData[i])
    //   }
    // }
    // this.gridApi1.setRowData(Newarray)
  }
}
