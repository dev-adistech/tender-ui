        ...
        cellRenderer: function (params) {
          ...
          if (op.PASS == op.PASSWORD) {
            if (op.ALLOWUPD) {
              a =
                a +
                '<svg class="grid-icon icon-save" data-action-type="SaveData" > <use  data-action-type="SaveData" xlink: href = "assets/symbol-defs.svg#icon-save" > </use> </svg>';
            }
          }
          a = a + "</span>";
          return a;
    ...
    if (eve.event.target !== undefined) {
      ...
      if (actionType == "SaveData") {
        ...
        this.LotMapMastServ.LotMappingSave({
          ...
          L_CODE: eve.data.L_CODE ? eve.data.L_CODE : "",
          C_SRNO: eve.data.C_SRNO ? eve.data.C_SRNO : 0,
          PER: eve.data.PER ? eve.data.PER : 0,
          ISWIN: eve.data.ISWIN ? eve.data.ISWIN : false,
        }).subscribe((DelRes) => {
          try {
