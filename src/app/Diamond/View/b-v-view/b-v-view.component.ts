...
export class BVViewComponent implements OnInit {
  ...
  AreaBoxStyle:string=`border:1px solid black;width: 406px;height: 100px;resize: none;`;
  ContainWidth:string=`border:1px solid black;width: 406px;height: 58px;border-top:none`;
  BlankBoxStyle:string=`border:1px solid black;padding: 21px 0px; width: 406px; text-align: center;border-top:none;`;
  HearderBoxStyle:string=`border:1px solid black; width:406px; text-align: center;border-bottom:none`;
  ...
  GetRowData(DetId,Srno,Comp){
    let NewRow =[]
      for(let j=0;j<this.NewRowData.length;j++){
        if(Comp === this.NewRowData[j].COMP_CODE && Srno === this.NewRowData[j].SRNO && DetId === this.NewRowData[j].DETID){
          NewRow.push(this.NewRowData[j])
        }
      }
      NewRow
  }
